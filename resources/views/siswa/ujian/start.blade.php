        ...
        <form action="{{ route('siswa.ujian.submit', $ujian->id_ujian) }}" method="POST">
            ...
            @foreach ($ujian->soalUjian as $index => $soal)
                <div class="mt-4">
                    ...
                    <div>
                        <label><input type="radio" name="jawaban_{{ $soal->id_soal_ujian }}" value="A"> {{ $soal->opsi_a }}</label><br>
                        <label><input type="radio" name="jawaban_{{ $soal->id_soal_ujian }}" value="B"> {{ $soal->opsi_b }}</label><br>
                        <label><input type="radio" name="jawaban_{{ $soal->id_soal_ujian }}" value="C"> {{ $soal->opsi_c }}</label><br>
                        <label><input type="radio" name="jawaban_{{ $soal->id_soal_ujian }}" value="D"> {{ $soal->opsi_d }}</label>
                    </div>
                </div>
            @endforeach
