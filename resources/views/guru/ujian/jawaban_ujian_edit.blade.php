<x-app-guru-layout>
    <div class="p-2 mx-4 my-6 bg-white rounded-lg shadow xl:p-6">
        ...
        <form action="{{ route('jawaban_ujian.update', $jawaban->id_jawaban_ujian) }}" method="POST">
            ...
            <div class="mb-4">
                <label for="soal_ujian_id" class="block text-sm font-medium text-gray-700">Soal Ujian</label>
                <select name="soal_ujian_id" id="soal_ujian_id" class="block w-full mt-1 border-gray-300 rounded-lg shadow-sm">
                    @foreach($soalUjian as $soal)
                        <option value="{{ $soal->id }}" {{ $jawaban->soal_ujian_id == $soal->id ? 'selected' : '' }}>
                            {{ $soal->teks_soal }}
                        </option>
                    @endforeach
                </select>
